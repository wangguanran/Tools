...
def display_reg0E_info_to_output(reg_value, result, log_line, output, use_colors=True):
    ...
    if use_colors:
        ...
        output.write(f"{Colors.BOLD}寄存器值: 0x{reg_value} (0b{binary_value}){Colors.END}\n")
    else:
        output.write(f"{log_line}\n")
        output.write(f"寄存器值: 0x{reg_value} (0b{binary_value})\n")
    
    # 显示VREG设置
    vreg_info = result['VREG']
    output.write("\n充电电压设置:\n")
    if use_colors:
        output.write(f"  VREG[4:0]: {Colors.GREEN}{vreg_info['binary']}{Colors.END}\n")
        output.write(f"  电压值: {Colors.CYAN}{vreg_info['voltage']}{Colors.END}\n")
        output.write(f"  {Colors.YELLOW}{vreg_info['description']}{Colors.END}\n")
    else:
        output.write(f"  VREG[4:0]: {vreg_info['binary']}\n")
        output.write(f"  电压值: {vreg_info['voltage']}\n")
        output.write(f"  {vreg_info['description']}\n")
    
    # 显示Reserved位
    reserved_info = result['Reserved']
    if use_colors:
        output.write(f"\nReserved位: {Colors.YELLOW}{format(reserved_info['value'], '02b')}{Colors.END} - {reserved_info['description']}\n")
    else:
        output.write(f"\nReserved位: {format(reserved_info['value'], '02b')} - {reserved_info['description']}\n")
    
    # 显示VRECHG设置
    vrechg_info = result['VRECHG']
    if use_colors:
        output.write(f"\n充电恢复电压: {Colors.GREEN}{vrechg_info['value']}{Colors.END} - {vrechg_info['description']}\n")
    else:
        output.write(f"\n充电恢复电压: {vrechg_info['value']} - {vrechg_info['description']}\n")
...
# 主要处理函数
def get_register_handler(reg, value):
    """根据寄存器地址获取对应的解析处理函数并执行解析
    
    Args:
        reg (str): 寄存器地址（如 "00", "0a" 等）
        value (str): 寄存器值（十六进制字符串）
        
    Returns:
        tuple: (解析结果字典, 显示函数)，如果寄存器不支持则返回 (None, None)
    """
    reg = reg.lower()
    parse_functions = {
        "00": parse_reg00,
        ...
        with open(log_file, 'r', encoding='utf-8') as f:
            ...
            for line in lines:
                ...
                if match:
                    reg = match.group(1)
                    value = match.group(2)
                    parse_func, display_func = get_register_handler(reg, value)
